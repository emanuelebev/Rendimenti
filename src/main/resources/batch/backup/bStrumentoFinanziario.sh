...
. promSettings.sh
export LOGFILE=${PATH_PFP_LOG}lStrumentoFinanziario.log


rm -f ${PFP_FLUSSI}ANAG_RUNCOM.txt
export LAST_ANAG_UPDATE=`ls -Atr ${PFP_FLUSSI}ANAGRAFE_* | tail -n 1`
cp -f $LAST_ANAG_UPDATE ${PFP_FLUSSI}ANAG_RUNCOM.txt
...
. ${PFP_BATCH}promBatchPFPNoAmbiente.sh ${PATH_FASI_XML}bStrumentoFinanziario.xml >> ${LOGFILE}
