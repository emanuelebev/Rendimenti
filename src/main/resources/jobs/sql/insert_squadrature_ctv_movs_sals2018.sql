...
IF check_ix_sals = 0 THEN
		EXECUTE IMMEDIATE 'CREATE INDEX IX_SALS ON RENDIMPC.SQUADRATURE_CTV_SALS (IDPTF, CODICETITOLO, DATA) TABLESPACE TBS_RENDIMPC_IDX_TEMP';
END IF;
...
SELECT COUNT(*) INTO check_ix_movs FROM user_indexes WHERE index_name = 'IX_MOVS' AND table_owner = 'RENDIMPC';
	...
	IF check_ix_movs = 0 THEN
			EXECUTE IMMEDIATE 'CREATE INDEX IX_MOVS ON RENDIMPC.SQUADRATURE_CTV_MOVS (IDRAPPORTO, CODICETITOLO) TABLESPACE TBS_RENDIMPC_IDX_TEMP';
	END IF;   
...
dbms_stats.gather_table_stats(ownname => sys_context('USERENV', 'CURRENT_SCHEMA'), tabname => 'SQUADRATURE_CTV_SALS', degree => 4, estimate_percent=>1);
dbms_stats.gather_table_stats(ownname => sys_context('USERENV', 'CURRENT_SCHEMA'), tabname => 'SQUADRATURE_CTV_MOVS', degree => 4, estimate_percent=>1);

INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' Statistiche 2018 eseguite');
	COMMIT;	
