...
-- maxyearmonth > t1mpz
-- a questo punto vengono travasate tutte le polizze presenti nella parcheggio con data <= t1mpz 
DECLARE 
...
CURSOR merge_saldi_parc IS
	SELECT /*+ PARALLEL(8) */ 
	        ...
	        WHERE (sp.idptf, sp.codicetitolo, sp.DATA) IN ( SELECT	srp.idptf           AS idptf,
												        			srp.codicetitolo    AS codicetitolo,
												        			MAX(srp.DATA) 		AS DATA
													FROM saldo_rend_polizze srp
													WHERE to_number(CONCAT(EXTRACT(YEAR FROM TO_DATE(srp.dataaggiornamento, 'YYYYMMDD' )), lpad(TRIM(EXTRACT(MONTH FROM TO_DATE(srp.dataaggiornamento, 'YYYYMMDD' ))),2,'0'))) = t1mpz
												    AND srp.tipo_pol = 'PZ'
												    GROUP BY srp.idptf, srp.codicetitolo)
...
INSERT (idptf,
	...
	IF MOD(I,10000) = 0 THEN
			    INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' MERGE SALDO_REND POLIZZE MONTH - COMMIT ON ROW: '|| I);
			COMMIT;
		END IF;
		...
		CLOSE merge_saldi_parc;
			...
			INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' MERGE SALDO_REND POLIZZE MONTH - COMMIT ON ROW: ' || totale);
		COMMIT;     
