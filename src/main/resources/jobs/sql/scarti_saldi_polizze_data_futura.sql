...
CURSOR /*+ parallel(8) */ scarti_saldi_polizze_multi IS
	SELECT sp.*, sp.ROWID
	FROM tmp_pfsaldi_polizze sp
		LEFT JOIN tbl_bridge b
			ON b.cod_universo = sp.codiceinterno
...
BEGIN
	...
	FOR cur_item IN scarti_saldi_polizze_mono
		...
		I := I+1;
			...
			IF MOD(I,10000) = 0 THEN
				INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFSALDI_POLIZZE MONO PER DATA FUTURA - COMMIT ON ROW: '|| I);
				COMMIT; 
			END IF;
					
		END LOOP;
			 INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFSALDI_POLIZZE MONO PER DATA FUTURA. RECORD ELABORATI: '|| I);
			 INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFSALDI_POLIZZE MONO PER DATA FUTURA. RECORD SCARTATI: '|| scarti_data);
		COMMIT;
	...
	FOR cur_item IN scarti_saldi_polizze_multi
		...
		I := I+1;
			...
			IF MOD(I,10000) = 0 THEN
				INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFSALDI_POLIZZE MULTI PER DATA FUTURA NON CENSITA - COMMIT ON ROW: '|| I);
				COMMIT; 
			END IF;
					
		END LOOP;
			 INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFSALDI_POLIZZE MULTI PER DATA FUTURA. RECORD ELABORATI: '|| I);
			 INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFSALDI_POLIZZE MULTI PER DATA FUTURA. RECORD SCARTATI: '|| scarti_data);
		COMMIT;
	END;