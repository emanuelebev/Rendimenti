...
BEGIN
	...
	FOR cur_item IN scarti_saldi_polizze_mono
		...
		I := I+1;
			...
			INSERT INTO 
					TBL_SCARTI_TMP_PFSALDI_POLI_SP (codicebanca,
													codiceagenzia,
													codicerapporto,
													codiceinterno,
													ramo,
					...
					DELETE /*+ nologging */ FROM tmp_pfsaldi_polizze_sp tmp_del
					WHERE tmp_del.ROWID = cur_item.ROWID;
					
					scarti_data := scarti_data +1;
			
			IF MOD(I,10000) = 0 THEN
				INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFSALDI_POLIZZE_SP MONO PER DATA FUTURA - COMMIT ON ROW: '|| I);
				COMMIT; 
			END IF;
					
		END LOOP;
			 INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFSALDI_POLIZZE_SP MONO PER DATA FUTURA. RECORD ELABORATI: '|| I);
			 INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFSALDI_POLIZZE_SP MONO PER DATA FUTURA. RECORD SCARTATI: '|| scarti_data);
		COMMIT;
	...
	FOR cur_item IN scarti_saldi_polizze_multi
		...
		I := I+1;
			...
			IF MOD(I,10000) = 0 THEN
				INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFSALDI_POLIZZE_SP MULTI PER DATA FUTURA NON CENSITA - COMMIT ON ROW: '|| I);
				COMMIT; 
			END IF;
					
		END LOOP;
			 INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFSALDI_POLIZZE_SP MULTI PER DATA FUTURA. RECORD ELABORATI: '|| I);
			 INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFSALDI_POLIZZE_SP MULTI PER DATA FUTURA. RECORD SCARTATI: '|| scarti_data);
		COMMIT;
	END;