...
-- maxyearmonthsp > t1msp
-- a questo punto vengono travasate tutte le polizze presenti nella parcheggio con data <= t1msp
DECLARE 
...
CURSOR merge_saldi_parc IS
	SELECT /*+ PARALLEL(8) */ 
	        ...
	        WHERE (sp.idptf, sp.codicetitolo, sp.DATA) IN ( SELECT	idptf,
														        	codicetitolo,
														        	MAX(DATA) 		AS DATA
															FROM saldo_rend_polizze 
															WHERE to_number(CONCAT(EXTRACT(YEAR FROM TO_DATE(dataaggiornamento, 'YYYYMMDD' )), LPAD(trim(EXTRACT(MONTH FROM TO_DATE(dataaggiornamento, 'YYYYMMDD' ))),2,'0'))) = t1msp
														    AND tipo_pol = 'SP'
...
INSERT (idptf,
	...
	IF MOD(I,10000) = 0 THEN
			    INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || '  MERGE SALDO_REND_POLIZZE SP POLIZZE MONTH - COMMIT ON ROW: '|| I);
			COMMIT;
		END IF;
		...
		CLOSE merge_saldi_parc;
			...
			INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || '  MERGE SALDO_REND_POLIZZE SP POLIZZE MONTH - COMMIT ON ROW: ' || totale);
		COMMIT;     
