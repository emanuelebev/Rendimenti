...
CURSOR SCARTI_RAPPORTI_DOPPI IS
	...
	FROM TMP_PFRAP TMP
	INNER JOIN (SELECT ID_BUONO_ORIGINARIO
		  FROM TMP_PFRAP TMP
		  WHERE ID_BUONO_ORIGINARIO IS NOT NULL
		  GROUP BY ID_BUONO_ORIGINARIO
		  HAVING COUNT(*) >1) A
...
BEGIN
	...
	IF CHECK_IDX = 0 THEN
	    EXECUTE IMMEDIATE 'CREATE INDEX RENDIMPC.IDX_RAP_SCARTI ON TMP_PFRAP(ID_BUONO_ORIGINARIO) TABLESPACE RENDIMPC_INDEX PARALLEL 8';
	      ...
	      TOTALE := TOTALE + I;
				...
				INSERT   INTO TBL_SCARTI_TMP_PFRAP (	CODICEAGENZIA,
												...
												VALUES (RES_SCARTI_RAPPORTI_DOPPI(J).CODICEAGENZIA,
														...
														'CHIAVE PRIMARIA "ID_BUONO_ORIGINARIO" DUPLICATA',
														SYSDATE
														);
	             ...
	             INSERT INTO OUTPUT_PRINT_TABLE VALUES (TO_NUMBER(TO_CHAR(SYSDATE,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFRAP PER CHIAVE (ID_BUONO_ORIGINARIO) DUPLICATA: ' || I || ' RECORD');
			        
	END LOOP;
	CLOSE SCARTI_RAPPORTI_DOPPI;
	
	INSERT INTO OUTPUT_PRINT_TABLE VALUES (TO_NUMBER(TO_CHAR(SYSDATE,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFRAP PER CHIAVE (ID_BUONO_ORIGINARIO) DUPLICATA: ' || TOTALE);
	COMMIT;
