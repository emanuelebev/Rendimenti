declare
    type deletedRows is table of TMP_PFMOV_LIBRETTI%rowtype;
    delRowsNoRapporto deletedRows;
    delRowsNoCodicetitolo deletedRows;
begin
	loop
		delete TMP_PFMOV_LIBRETTI t 
		...
		if delRowsNoCodicetitolo.count = 0 THEN
			...
			forall i in 1..delRowsNoCodicetitolo.count
				INSERT INTO tbl_scarti_TMP_PFMOV_LIBRETTI
					(
						CODICEBANCA,
						CODICE,
						CODICEAGENZIA,
	...
	commit;
	  ...
	  INSERT INTO CAUSALE (	  CODICECAUSALE,
	         ...
	         t.tiporapporto, --CANALE
               ...
               FROM TMP_PFMOV_LIBRETTI t
               WHERE NOT EXISTS
               (SELECT 1
                FROM CAUSALE c
                WHERE c.CODICECAUSALE = t.TIPORAPPORTO || '_' || t.CAUSALE);
   
      INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' SCARTI TMP_PFMOV_LIBRETTI FK NON CENSIITA.');
	COMMIT;
end;