...
CURSOR merge_saldi_parc IS
	SELECT  idptf                         AS idptf,
        ...
        WHERE (idptf, codicetitolo, DATA) IN (	SELECT	idptf,
														...
														MAX(DATA) 		AS DATA
												FROM saldo_rend_sott_pol 
												WHERE to_number(CONCAT(EXTRACT(YEAR FROM TO_DATE(dataaggiornamento, 'YYYYMMDD' )), lpad(TRIM(EXTRACT(MONTH FROM TO_DATE(dataaggiornamento, 'YYYYMMDD' ))),2,'0'))) = t1mpz
...
BEGIN
	...
	IF MOD(I,10000) = 0 THEN
			    INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' MERGE SALDO_REND_SOTT POLIZZE MONTH - COMMIT ON ROW: '|| I);
			COMMIT;
		END IF;
		...
		CLOSE merge_saldi_parc;
			...
			INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' MERGE SALDO_REND_SOTT POLIZZE MONTH - COMMIT ON ROW: ' || totale);
		COMMIT;     
