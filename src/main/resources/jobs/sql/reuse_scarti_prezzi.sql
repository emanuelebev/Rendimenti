...
FROM tbl_scarti_tmp_pfprezzi scarto
LEFT JOIN tmp_pfprezzi tmp
...
BEGIN 
	...
	IF (cur_item.tmp_rowid IS NULL) THEN 
		...
		DELETE /*+ nologging */ FROM tbl_scarti_tmp_pfprezzi del_scarto
		WHERE del_scarto.ROWID = cur_item.ROWID;	
	
	IF MOD(I,10000) = 0 THEN
		INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' REUSE SCARTI TMP_PFPREZZI - COMMIT ON ROW: '|| I);
		COMMIT; 
		END IF;
		
	END LOOP;
	INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' REUSE SCARTI TMP_PFPREZZI - RECORD ANALIZZATI: '|| I || '; RECORD RIUTILIZZATI: ' || count_reuse_prezzi);
	COMMIT;
END; 