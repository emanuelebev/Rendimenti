--Da fare su Kettle:
--drop index IX_RAPPORTO_PZ1;
--create index IX_RAPPORTO_PZ1 on RAPPORTO (codicerapporto, tipo) TABLESPACE RENDIMPC_INDEX;
...
SELECT COUNT(*) INTO CNT FROM TMP_PFSALPOL_RECUP;
...
END IF;
	...
	IF MOD(I,10000) = 0 THEN
		    INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' MERGE SALDO_REND POLIZZE MONORAMO - COMMIT ON ROW: '|| I);
		COMMIT;
	END IF;
	...
	CLOSE SALPO_MONO;
		...
		INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' MERGE SALDO_REND POLIZZE MONORAMO - COMMIT ON ROW: ' || totale);
	COMMIT;     
 ...
 OPEN SALPO_MULTI; --Saldi polizze multiramo
	...
	IF MOD(I,10000) = 0 THEN
		    INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' MERGE SALDO_REND POLIZZE MULTIRAMO - COMMIT ON ROW: '|| I);
		COMMIT;
	END IF;
	...
	CLOSE SALPO_MULTI;
		...
		INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' MERGE SALDO_REND POLIZZE MULTIRAMO - COMMIT ON ROW: ' || totale);
	COMMIT;
END;