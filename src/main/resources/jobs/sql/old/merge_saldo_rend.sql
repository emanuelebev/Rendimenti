...
BEGIN
	...
	INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' MERGE SALDO_REND - COMMIT ON ROW: ' || totale);
	COMMIT;
			...
			FORALL J IN RES_saldi_rend_bfpd.FIRST .. RES_saldi_rend_bfpd.LAST	
				...
				MERGE INTO
					saldo_rend sr
						USING (
								SELECT
									RES_saldi_rend_bfpd(j).idptf								AS idptf,
									RES_saldi_rend_bfpd(j).codicebanca						AS codicebanca,
									RES_saldi_rend_bfpd(j).codicefiliale 					AS codicefiliale,
									RES_saldi_rend_bfpd(j).codicerapporto					AS codicerapporto,
									RES_saldi_rend_bfpd(j).codicetitolo 						AS codicetitolo,
									RES_saldi_rend_bfpd(j)."DATA" 							AS "DATA",
									RES_saldi_rend_bfpd(j).ctv 								AS ctv,
									RES_saldi_rend_bfpd(j).ctvvaluta 						AS ctvvaluta,
									RES_saldi_rend_bfpd(j).dataaggiornamento 				AS dataaggiornamento,
									RES_saldi_rend_bfpd(j).divisa 							AS divisa,
									RES_saldi_rend_bfpd(j).flag_consolidato 					AS flag_consolidato,
									RES_saldi_rend_bfpd(j).id_rapporto 						AS id_rapporto,
									RES_saldi_rend_bfpd(j).ctv_versato 						AS ctv_versato,
									RES_saldi_rend_bfpd(j).ctv_prelevato 					AS ctv_prelevato,
									RES_saldi_rend_bfpd(j).ctv_versato_netto 				AS ctv_versato_netto,
									RES_saldi_rend_bfpd(j).codicerapporto_ori 				AS codicerapporto_ori,
									RES_saldi_rend_bfpd(j).cod_fasciarendimento_bfp 			AS cod_fasciarendimento_bfp,
									RES_saldi_rend_bfpd(j).descr_fasciarendimento_bfp 		AS descr_fasciarendimento_bfp,
									RES_saldi_rend_bfpd(j).serie_bfp 						AS serie_bfp,
									RES_saldi_rend_bfpd(j).valore_scadenza_lordo_bfp 		AS valore_scadenza_lordo_bfp,
									RES_saldi_rend_bfpd(j).flag_lordista_bfp 				AS flag_lordista_bfp,
									RES_saldi_rend_bfpd(j).importo_netto_bfp 				AS importo_netto_bfp,
									RES_saldi_rend_bfpd(j).valore_scadenza_netto_bfp 		AS valore_scadenza_netto_bfp,
									RES_saldi_rend_bfpd(j).valore_nominale_bfp 				AS valore_nominale_bfp,
									RES_saldi_rend_bfpd(j).datasottoscrizione 				AS datasottoscrizione,
									RES_saldi_rend_bfpd(j).datascadenza						AS datascadenza
								FROM dual
								)T
					 ON(sr.idptf = t.idptf
						AND sr.codicetitolo = t.codicetitolo
		...
		INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' MERGE SALDO_REND BFPD - COMMIT ON ROW: '|| I);
		COMMIT;
	...
	INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' MERGE SALDO_REND BFPD - COMMIT ON ROW: ' || totale);
	COMMIT;
END;