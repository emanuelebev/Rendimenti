-- Le seguenti istruzioni per la creazione e rimozione degli indici sono state inserite nel job di kettle:
-- drop index RAP_PTF_IDX_2;
-- create unique index RAP_PTF_IDX_2 on RAPPORTO (id,IDPTF) TABLESPACE RENDIMPC_INDEX;
-- drop index SALDO_IDX_2;
-- create unique index SALDO_IDX_2 on SALDO (IDPTF, codicetitolo, data, valore) TABLESPACE RENDIMPC_INDEX;
-----------------------------------------------------------------------------------------------------------
...
FROM (
    ...
    AND M.DATA <= (SELECT MAX(DATA) FROM saldo) 
    AND R.tipo != '02'
...
BEGIN	
	...
	LOOP
		...
		INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' Squadrature QTA no libretti: ' || I || ' record'); 
		COMMIT;		        
	...
	INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' Squadrature QTA no libretti: ' || totale);
	COMMIT;
	...
	LOOP
		...
		INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' Squadrature QTA libretti: ' || I || ' record'); 
		COMMIT;		        
	...
	INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' Squadrature QTA  libretti: ' || totale);
	COMMIT;

	
	--Cancello squadrature rientrate
		DELETE FROM squadrature
		WHERE dataagg IS NOT NULL
		AND TRUNC(sysdate) - TRUNC(dataagg) >= 1;
		
		count_deleted := count_deleted +  SQL%rowcount;
		
		INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' Squadrature QTA. Squadrature eliminate: ' || count_deleted);
		COMMIT;
		
	ELSE 
	INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' Squadrature QTA non eseguite, la tabella SALDO è vuota.');
		COMMIT;
