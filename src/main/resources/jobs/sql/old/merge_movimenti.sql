...
MERGE INTO
  ...
  USING (
          SELECT
            ...
            CASE
				...
				ELSE TMOV.CODICEINTERNO END                                                    				 AS CODICETITOLO,
            ...
            CASE 
            	...
            	ELSE NULL END 																				 AS CODICERAPPORTO_ORI,
            TMOV.CAUSALE																					 AS CAUSALE_ORI,
            CASE --per update ctvregolato quote carico e scarico
            	WHEN TMOV.CAUSALE IN ('ESS', 'QXS', 'TCS', 'QCS', 'FDS', 'ECS', 'QYS', 'TSS', 'QSS', 'FCS') 
            			AND TMOV.CTVREGOLATO = 0 THEN 'PDR' END												 AS FONTE
          FROM
  ...
  INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' MERGE MOVIMENTI: ' || rowcounts);
	COMMIT;
