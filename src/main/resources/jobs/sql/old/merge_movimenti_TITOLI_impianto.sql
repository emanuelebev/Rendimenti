...
VAR_PERC := 26/100;
 ...
 FOR CUR_ITEM IN cerca_mov
	...
	exception when others then 
		INSERT INTO OUTPUT_PRINT_TABLE VALUES (TO_NUMBER(TO_CHAR(SYSDATE,'YYYYMMDDHH24MISS')) || ' merge TMP_PFMOV_TITOLI - err per idrapporto: '|| cur_item.idrapporto || ' numreg: ' || cur_item.numreg);
	end;

IF MOD(I,10000) = 0 THEN
		    INSERT INTO OUTPUT_PRINT_TABLE VALUES (TO_NUMBER(TO_CHAR(SYSDATE,'YYYYMMDDHH24MISS')) || ' merge TMP_PFMOV_TITOLI - COMMIT ON ROW: '|| I);
		COMMIT;
		END IF;
	END LOOP;
	INSERT INTO OUTPUT_PRINT_TABLE VALUES (TO_NUMBER(TO_CHAR(SYSDATE,'YYYYMMDDHH24MISS')) || ' merge TMP_PFMOV_TITOLI - COMMIT ON ROW: ' || I);
