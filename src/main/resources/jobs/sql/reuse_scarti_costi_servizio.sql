...
CURSOR del_cur IS
	...
	FROM TBL_SCARTI_TMP_PFCOSTI_SERV scarto
	LEFT JOIN TMP_PFCOSTI_SERV tmp
		ON tmp.codicebanca = scarto.codicebanca
			...
			AND tmp.datavaluta = scarto.datavaluta
	...
	LOOP
		...
		IF(cur_item.tmp_rowid IS NULL) THEN
				...
				INSERT /*+ append nologging parallel(8) */
						INTO TMP_PFCOSTI_SERV (	codicebanca,
												codicecliente,
												codicemovimento,
												codicecosto,
												codiceagenzia,
			...
			DELETE /*+ nologging */ FROM TBL_SCARTI_TMP_PFCOSTI_SERV tbl
			WHERE tbl.ROWID = cur_item.scarto_rowid;
						
	IF MOD(I,10000) = 0 THEN
		INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' REUSE SCARTI TMP_PFCOSTI_SERV. COMMIT ON ROW: ' || I);
		COMMIT;
	END IF;

	END LOOP;
	
INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' REUSE SCARTI TMP_PFCOSTI_SERV. RECORD RECUPERATI: ' || reuse_count);
	COMMIT;
