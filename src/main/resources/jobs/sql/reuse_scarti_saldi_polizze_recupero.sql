...
CURSOR DEL_CUR IS
	...
	FROM TBL_SCARTI_TMP_PFSALPOL_RECUP SCARTO
	LEFT JOIN TMP_PFSALPOL_RECUP TMP
...
BEGIN
	...
	FOR CUR_ITEM IN DEL_CUR 
		...
		INSERT /*+ append nologging parallel(8) */
				INTO TMP_PFSALPOL_RECUP (	CODICEBANCA,
											CODICEAGENZIA,
											CODICERAPPORTO,
											CODICEINTERNO,
											RAMO,
			...
			DELETE /*+ nologging */ FROM TBL_SCARTI_TMP_PFSALPOL_RECUP TBL
			WHERE TBL.ROWID = CUR_ITEM.SCARTO_ROWID;
						
		IF MOD(I,10000) = 0 THEN
			INSERT INTO OUTPUT_PRINT_TABLE VALUES (TO_NUMBER(TO_CHAR(SYSDATE,'YYYYMMDDHH24MISS')) || ' REUSE SCARTI TMP_PFSALPOL_RECUP. COMMIT ON ROW: ' || I);
			COMMIT;
		END IF;

		END LOOP;
			INSERT INTO OUTPUT_PRINT_TABLE VALUES (TO_NUMBER(TO_CHAR(SYSDATE,'YYYYMMDDHH24MISS')) || ' REUSE SCARTI TMP_PFSALPOL_RECUP. RECORD RECUPERATI: ' || REUSE_COUNT);
		COMMIT;
