...
--Movimenti polizze multiramo premium
CURSOR cerca_movmulti IS 
	SELECT 	/*+ PARALLEL(8) */	  
			...
			CASE WHEN ramo = '01' THEN r.codicetitolo_multiramo || 'RI'
				 WHEN ramo = '03' THEN r.codicetitolo_multiramo || 'RIII' 
			ELSE r.codicetitolo_multiramo END																					AS codicetitolo,
			premio_lordo_coass																									AS ctv,
			premio_lordo_coass																									AS qta,
			...
			codice_tipo_contributo																									AS codice_tipo_contributo,
			'N'																														AS f_cancellato						
...
BEGIN
	...
	FOR cur_item IN cerca_movmulti
		...
		MERGE INTO movimento_ramo_terzo mov
		  USING (SELECT cur_item.idrapporto          				AS idrapporto,
						...
						cur_item.codice_tipo_contributo				AS codice_tipo_contributo,
	...
	WHEN MATCHED THEN UPDATE
		  SET
		    ...
		    mov.codice_tipo_contributo = tomerge.codice_tipo_contributo,
	...
	WHEN NOT MATCHED THEN 
			INSERT (idrapporto,
  			...
  			VALUES (tomerge.idrapporto,
					...
					tomerge.costo_etf,
					tomerge.codice_tipo_contributo,
					tomerge.f_cancellato
				);
	
	IF MOD(I,10000) = 0 THEN
		    INSERT INTO output_print_table VALUES (to_number(to_char(sysdate,'YYYYMMDDHH24MISS')) || ' MERGE TMP_PFMOV_INC_COASS PREMIUM - COMMIT ON ROW: '|| I);
		COMMIT;
	END IF;
